...
export const useMtkMap = () => {
    ...
    const initDraw = (snap: any, vtLayer: any) => {
        ...
        function clearZoomCache() {
            // 清除地图渲染缓存 全部清除 会影响工具的样式设置的显示
            // if (map.getRenderer()){
            //     map.getRenderer().clearCache();
            // }
            
            // 清除矢量瓦片图层缓存
            if (vtLayer) {
                vtLayer.getRenderer().render(); // 刷新图层数据
            }
        ...
        let items = ['LineString', 'Polygon', 'Point'].map(function (value) {
            return {
                item: value,
                click: function () {
                    // 在绘制开始前清除缩放缓存
    ...
    const initMap = () => {
        ...
        initHighLight(map, vtLayer)// ... existing code ...
    }
